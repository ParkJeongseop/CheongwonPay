...
public class CheongwonPayServer {
    ...
    public static void main(String[] args) throws IOException {
        ...
        try {
			con = DriverManager.getConnection("jdbc:mysql://localhost/cheongwonpaydb", "testuser", "testuserpassword");// DB서버(MySQL)주소와 계정, 패스워드
        ...
        while(true){
           ...
           if(readOPData == OP_LOGIN){// 로그인
                try {
                 ...
                 if(password!= null && password.equals(PW)){//성공했을때
           ...
           if(readOPData == OP_PURCHASE){// 거래, 출석체크
              try {
            	  readData = dis.readUTF();// 클라이언트가 보낸 데이터를 "readData"에 저장한다.
                  ...
                  if(User.equals("null")){// 바코드데이터가 null일 때
                	  ...
                	  if(simplifiedFDS(School_Type)){// 축제시간일 때
                          ...
                          // 거래가능조건 잔액>=상품가
                          // 결제기능 주석화로 제거 if(Balance>=Price){
                              // 거래내역추가
                              st.execute("INSERT INTO transactions (User, Club_Num, Goods_Num) VALUES ('"+User+"',"+Club_Num+","+Goods_Num+")");         
                              ...
                              if(currentTime.compareTo(LastCheckT) > 0){// 마지막 출석체크 된 시간 +10분 보다 현재시간이 더 클 때
                            	  if (st.execute("SELECT Time FROM atd_history where User='"+User+"'and Club_Num ="+Club_Num+"")) {// 이 동아리에서의 학생의 결제기록 불러오기
                            		  rs = st.getResultSet();
                            	  }
           ...
           if(readOPData == OP_ADD_ITEM){// 상품추가
               try {
                   ...
                   if(!simplifiedFDS(School_Type)){// 축제시간 이외일 때
                       ...
                       java.sql.Statement st = null;
                       st = CheongwonPayServer.con.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
                       st.execute("INSERT INTO goods (Club_Num, Goods_Name, Price) VALUES ('"+Club_Num+"','"+Goods_Name+"','"+Price+"')");// 데이터베이스에 동아리고유번호, 상품명, 상품가로 데이터를 저장한다.
                   }
             } catch (IOException e) {
                e.printStackTrace();
             }  catch (SQLException ex) {
...
*/
                  while (rs.next()) {
                	  ...
                	  switch(rs.getString("School")){// 남고,여고의 출석체크기준이 다르기때문에 swich문 사용
                	  case "M":// 남고일 때
           ...
           if(readOPData == OP_GetGoodsList){// 상품목록불러오기    goodsnum,name,price주기
               try {
                   ...
                   while (rs.next()) {
                       dos.writeUTF(rs.getInt("Goods_Num") + ":" + rs.getString("Goods_Name") + ":" + rs.getInt("Price"));// 상품고유번호, 상품명, 상품가를 전송
                   }
           ...
           if(readOPData == OP_GetRefundList){//get사용자결제목록 for refund    time,goodsnum,name,price주기
               try {
                   ...
                   if (st.execute("SELECT * FROM transactions where Club_Num='"+Club_Num+"' and User='"+User+"'")) {
                       rs = st.getResultSet();
                   }
           ...
           if(readOPData == OP_ATD){// 출석체크 부원등록
              try {
                   ...
                   st.execute("UPDATE user set Club_Num='"+Club_Num+"' where User='"+readData+"'");// 바코드에 일치하는 학생에 동아리 고유번호저장하기
              } catch (IOException e) {
               e.printStackTrace();
            }  catch (SQLException ex) {
                   Logger.getLogger(User.class.getName()).log(Level.SEVERE, null, ex);
           ...
           if(readOPData == OP_EditGoods){//상품목록변경
               try {
                    ...
                    if(!simplifiedFDS(School_Type)){// 축제시간 이외일 때
                        ...
                        st.execute("UPDATE goods set Goods_Name='"+Goods_Name+"', Price='"+Price+"' where Goods_Num='"+Goods_Num+"'");
                    }
               } catch (IOException e) {
                e.printStackTrace();
             }  catch (SQLException ex) {
           ...
           if(readOPData == OP_UserMatching){//유저매칭
               try {
                   ...
                   switch(Type){
                   case "1":// 남고일 때
                	   st.execute("UPDATE user set User='"+Barcode+"' where Grade='"+Grade+"' and Class='"+Class+"' and Number='"+Number+"' and School='M'");
                	   break;
                   case "2":// 여고일 때
                	   st.execute("UPDATE user set User='"+Barcode+"' where Grade='"+Grade+"' and Class='"+Class+"' and Number='"+Number+"' and School='W'");
                	   break;
                   }
             } catch (IOException e) {
                e.printStackTrace();
